...
def sector(x, y, rx, ry, start=0, extent=200):
    '''
    Рисование сектора
    x - x центра
    y - y центра
    rx -радиус по x
    ry - радиус по y (если сектор окружности, то rx == ry)
    start - смещение в градусах точки начала дуги сектора
    excent - угловая величина сектора
    '''
    x1 = x - rx
    y1 = y - ry
    x2 = x + rx
    ...
    x1, y1 = transformCoord(x1, y1)
    x2, y2 = transformCoord(x2, y2)
    sector = _C.create_arc(x1, y1, x2, y2, start=start, extent=extent,
                           outline=_penColor, width=_penSize, fill=_brushColor)
    return sector

def arc(x, y, rx, ry, start=0, extent=359):
    '''
    Рисование дуги
    x - x центра
    y - y центра
    rx -радиус по x
    ry - радиус по y (если дуга окружности, то rx == ry)
    start - смещение в градусах точки начала дуги
    excent - угловая величина дуги
    '''
    x1 = x - rx
    y1 = y - ry
    x2 = x + rx
    ...
    x1, y1 = transformCoord(x1, y1)
    x2, y2 = transformCoord(x2, y2)
    arc = _C.create_arc(x1, y1, x2, y2, start=start, extent=extent, style=tkinter.ARC,
                        outline=_penColor, width=_penSize, fill=_brushColor)
    return arc

def chord(x, y, rx, ry, start=0, extent=120):
    '''
    Рисование хорды
    x - x центра
    y - y центра
    rx -радиус по x
    ry - радиус по y (если дуга окружности, то rx == ry)
    start - смещение в градусах точки начала дуги хорды
    excent - угловая величина дуги хорды
    '''
    x1 = x - rx
    y1 = y - ry
    x2 = x + rx
    ...
    x1, y1 = transformCoord(x1, y1)
    x2, y2 = transformCoord(x2, y2)
    chord = _C.create_arc(x1, y1, x2, y2, start=start, extent=extent, style=tkinter.CHORD,
                          outline=_penColor, width=_penSize, fill=_brushColor)
    return chord
...
if __name__ == "__main__":
    ...
    sector(150, 100, 50, 50, start=0, extent=45)

    arc(50, 200, 50, 50, start=160, extent=-70)

    chord(250, 200, 50, 50, start=240, extent=100)
